...
def check_platform():
    ...
    if cpxsys in ('x86-64_linux', 'ppc64le_linux', 's390x_linux'):
        expectedsys = ('Linux',)
    elif cpxsys in ('x86-64_osx','arm64_osx'):
        expectedsys = ('Darwin',)
    ...
    if pysys not in expectedsys:
        raise Exception(
            "this 'cplex' package (version 22.1.1.0) is not compatible " +
...
def get_data():
    ...
    if is_windows():
        ...
        if is_mac():
            ext = '.so'
            extra = ['libcplex2211.dylib']
        else:
            ext = '.so'
            extra = ['libcplex2211.so']

    data = ['py{0:d}{1:d}_cplex2211{2:s}'.format(major, minor, ext)]
    data.extend(extra)
    return data
